...
if __name__ == '__main__':
    ...
    if ext == 'wav':
        for filepath in iglob(os.path.join(src_dir, "*.wav")):
            ...
            #print(filepath)
            #print("Processing file {}...".format(os.path.splitext(os.path.basename(filepath))[0]))
            ...
            np.save(os.path.join(trg_dir, filename + "_f0.npy"), np.hstack([bap, lf0[:, np.newaxis], vuv[:, np.newaxis]]))
            #np.save(os.path.join(trg_dir, filename + ".npy"), mfcc)  # Only save the MFCCs for now
            
            """Se ha comentado la linea 168 y se ha descomentado la 167 para almacenar el resto de parámetros
            Para esta función, únicamente se almacenan las 3 columnas que contienen bap, lf0 y vuv"""
            
    elif ext == 'npy':
        for filepath in iglob(os.path.join(src_dir, "*.npy")):
            print("Processing file {}...".format(os.path.basename(filepath)))
            x = np.load(filepath)
            """Cargamos el fichero que contiene el f0 que necesitamos"""
            f0_name = os.path.splitext(os.path.basename(filepath))[0] #Obtenemos sólo el nombre del archivo
            #Se ha modificado el código para que también tome en la entrada el directorio de f0
            x_f0 = np.load(os.path.join('data\PMA\Arctic\JG\mfcc_f0',f0_name) + '_f0.npy') #Cargamos el fichero que contiene los f0
            mfcc = x
            nframes = x.shape[0]
            ...
            # mfcc = x[:, :opt.num_mfcc]
            bap = x_f0[:, 0].reshape((-1, 1))
            lf0 = x_f0[:, 1].reshape((-1, 1))
            vuv = x_f0[:, 2].reshape((-1, 1))
            #Igualamos el número de frames de todos
            minimo = int(min(bap.shape[0],lf0.shape[0],vuv.shape[0]))
            bap = bap[0:minimo]
            lf0 = lf0[0:minimo]
            vuv = vuv[0:minimo]
            ...
            #Igualamos el numero de frames entre f0,sp y ap
            minimo = int(min(f0.shape[0],sp.shape[0],ap.shape[0]))
            f0 = f0[0:minimo]
            sp = sp[0:minimo]
            ap = ap[0:minimo]
